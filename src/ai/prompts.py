...
def analyze_candidate_user_prompt(
    ...
    return (
        ...
        "* WORKED EXAMPLE (Gen Digital rebrand vs career progression): "
        "the candidate may have BOTH 'Junior Software QA Engineer @ Avast "
        "Software (04/2022 - 06/2023)' from LinkedIn AND 'Software QA "
        "Engineer @ Gen Digital - Trust Based Solutions (Norton, Avast, "
        "AVG, ...) (06/2023 - 07/2025)' from CV/LinkedIn. 'Avast Software' "
        "got rebranded to 'Gen Digital' so the company is technically the "
        "same employer, BUT the titles 'Junior' and (mid) 'Software QA "
        "Engineer' represent a real promotion. Emit TWO experience rows: "
        "the Junior one with `source='linkedin'` and the mid one with "
        "`source='both'` (or whichever sources actually carried it). NEVER "
        "merge them just because Avast appears as a brand inside the Gen "
        "Digital subtitle. The same applies to any subsequent 'Senior "
        "Software QA Engineer @ Gen Digital (07/2025 - present)' row - "
        "that is a third, separate progression entry.\n\n"
...
def resume_user_prompt(
    ...
    return (
        ...
        "and distinct title. Never merge them into one entry. Example: "
        "'Junior Software QA Engineer (04/2022 - 06/2023)' and 'Software QA "
        "Engineer (07/2023 - 06/2025)' at the same company MUST appear as "
        "two separate rows.\n"
        "CV TAILORING RULES:\n"
        "These rules govern how the resume must be structured:\n"
        "Rule 1: Dates are mandatory on every experience, education and "
        "certification entry. Use the period field.\n"
        "Rule 2: Never invent experiences or delete entries that would create "
        "timeline gaps. Every real entry from the candidate profile must "
        "appear in the output.\n"
        "Rule 3: Classify each position by relevance to the target job: "
        "directly related (full detail, 3-5 bullets), partially related "
        "(moderate detail, 2-3 bullets), or unrelated (minimal detail, 1 "
        "bullet). Place directly related positions first.\n"
        "Rule 4: The professional_summary / headline must mention the target "
        "position title and be tailored to the specific job. Do not use a "
        "generic headline.\n"
        "Rule 5: The professional summary must be 2-4 sentences that "
        "highlight the candidate's strongest qualifications for THIS role.\n"
        "Rule 6: Order technical skills by relevance to the target position. "
        "Group by category when the list is long (e.g. Languages, Frameworks, "
        "Tools, Platforms). The most job-relevant skills come first.\n"
        "Rule 7: Career progression must be preserved as separate entries "
        "with individual dates. Junior, Mid, Senior at the same company are "
        "distinct rows showing growth.\n"
        "Rule 8: For IT roles, the candidate's main IT employer (e.g. Gen "
        "Digital) should be the most detailed experience section with rich "
        "bullets showcasing technical depth.\n"
        "Rule 9: Key internal projects (e.g. internal QA toolkit, automation "
        "frameworks) should be highlighted as bullets under the relevant "
        "experience entry when the role is technical.\n"
        "Rule 10: Quantify achievements where the data supports it (reduced "
        "bugs by X%, automated N test cases, managed team of N). Do not "
        "invent numbers.\n"
        "Rule 11: For senior roles, highlight mentoring, leadership, and "
        "cross-team collaboration explicitly.\n"
        "Rule 12: For each experience entry, lead with the most impactful "
        "bullet - the one that best aligns with the target job requirements.\n"
        "Rule 13: Education entries must include the institution, degree/field "
        "and period. If the candidate studied IT-related subjects, mention "
        "them.\n"
        "Rule 14: Languages use CEFR levels only (already covered above).\n"
        "Rule 15: Certifications and courses should be listed with the "
        "issuing body when known.\n"
        "Rule 16: The resume must be ATS-friendly: use standard section "
        "headings, avoid tables/columns/graphics in the data, include "
        "keywords from the job posting naturally in bullets.\n"
        "Rule 17: Keep the resume concise. 1-2 pages for junior/mid, up to "
        "3 pages for senior candidates with extensive relevant experience.\n"
        "Rule 18: Contact information should be on a single line: email | "
        "phone | location. LinkedIn and GitHub as separate fields.\n"
        "Rule 19: Never fabricate, exaggerate or hallucinate. Every claim "
        "must be traceable to the candidate's actual inputs.\n\n"
        "JOB:\n" + _dump_job(job) + "\n\n"
        "CANDIDATE:\n" + _dump(candidate) + "\n\n"
        "USER ANSWERS:\n" + _dump(answers) + "\n\n"
        "EVIDENCE:\n" + _dump(evidence) + "\n\n"
